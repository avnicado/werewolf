...
#def potion_effect(player, creatures, m, objects, global_objects, screen, global_cs, self):
 #   randpotion = randint(0, 3)
  #  if randpotion == 0:
   #     player.hp += 2
    #    globals.news.append("you drank a healing potion. glug, glug")
     #   if player.hp > player.hp_limit:
      #      player.hp = player.hp_limit
       #     globals.news.append("but you couldn't heal much")
   # if randpotion == 1:
    #    player.fullness += 60.0
     #   globals.news.append(" a sandwich")
    #if randpotion == 2:
      #  player.hp += 1.0
     #   globals.news.append("you drank a strength potion. glug, glug")
    #if randpotion == 3:
     #   player.invisibility_timer = 16
...
def apple_effect(player, creatures, m, objects, global_objects, screen, global_cs, self):
    ...
    if randint(0,5) == 0:
        globals.news.append("you ate an golden apple!!! munch munch")
        player.fullness += 60.0
    else:
    ...
    for x in range(5):
        ...
        if cl != []:
            ...
            if c.icon == "v":
                ...
                creatures.remove(c)
                globals.news.append("you killed a guard why?")
        ...
        if ci.lower() in "abcdefghijklmnopqrstuvwxyz ": 
            ...
            if selected[3] <= player.gold:
                if randint(0,3) != 0:
                    ...
                    globals.news.append("sorry your product could not be deliverd NO REFUNDS")
